...
/// <summary>
/// Logo upload validation and SVG sanitisation on
/// <see cref="OrganizationBrandingService"/>. Renaming — the other half of the
/// branding surface — is covered in <see cref="OrganizationIdentityTests"/>.
/// </summary>
public sealed class OrganizationBrandingServiceTests : IDisposable
{
    ...
    public async Task UploadLogo_rejects_oversized_payload()
    {
        await using var ctx = _db.NewContext();
        var svc = _db.NewOrganizationBrandingService(ctx);
        var oversized = new byte[OrganizationBrandingService.MaxLogoBytes + 1];
    ...
    public void SanitiseLogo_strips_script_tags_and_event_handlers_from_svg()
    {
        ...
        var sanitised = OrganizationBrandingService.SanitiseLogo("image/svg+xml", bytes);
    ...
    private static string Sanitise(string svg) =>
        System.Text.Encoding.UTF8.GetString(OrganizationBrandingService.SanitiseLogo(
    ...
    public void SanitiseLogo_leaves_png_bytes_untouched()
    {
        ...
        OrganizationBrandingService.SanitiseLogo("image/png", png).Should().Equal(png);
    }
}
