...
/// table/page/report triggers (<c>OnInsert</c>, <c>OnOpenPage</c>, …)
/// fall into the object-level <c>TRIGGERS</c> section.
/// </summary>
public static class SourceFileOutlineGrouper
{
    ...
    public static List<OutlineGroup> Build(IReadOnlyList<SourceFileOutlineItem> items, string? filter)
    {
        ...
        // Track which field is "active" so a field-bound trigger nests
        // under it. When the parent field is filtered out, the trigger
        // drops to the object-level triggers section so the user can
        // still find it by name.
        ...
        foreach (var item in items)
        {
            ...
            switch (item.Kind)
            {
                ...
                case "trigger":
                    var fieldBound = currentField is not null
        ...
        var groups = new List<OutlineGroup>(7);
