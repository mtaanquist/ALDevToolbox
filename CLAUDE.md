...
> **Phase 3 note.** Milestones P3.13 and P3.14 have landed: organisations, accounts, and per-org configuration are real. The logo and admin-defined always-included files now live in the database (`organization_assets`, `organization_files`); the ruleset and `.gitignore` template stay as embedded resources.
...
- **The ruleset and `.gitignore` ship as code.** They live as embedded resources under `Resources/` because they're per-deployment policy. Per-folder example AL file *contents* live in the `template_files` table and are admin-editable. Binary files inside template folders are out of scope for v1 — text content only. The logo, organisation defaults block, and always-included file list moved into the database in M14 (`organization_assets`, `organization_settings`, `organization_files`).
