...
> **Phase 3 note.** Milestone P3.13 has landed: organisations and accounts are real. `milestones.md` still slates the logo and per-org always-included files for M14 (they remain embedded resources for now). The ruleset and `.gitignore` template stay as embedded resources.

- **The SQLite database is the only persistence layer for templates, modules, the catalogue, per-folder file contents, organisations, users, signup requests, password reset tokens and login attempts.** Both authoring surfaces (the structured admin form and the TOML editor) write through the same `TemplateInput` pipeline into the DB. `Templates.seed/` is a one-time bootstrap for an empty *organisation* and an export target — nothing watches it at runtime, nothing writes back to it, and there is no two-way sync.
- **The ruleset and `.gitignore` ship as code.** They live as embedded resources under `Resources/` because they're per-deployment policy. Per-folder example AL file *contents* live in the `template_files` table and are admin-editable. Binary files inside template folders are out of scope for v1 — text content only. (M14 will move the logo and per-org always-included files into the DB.)
...
- Icons: Lucide.Blazor only. No mixing icon families. The same icon name should be used for the same concept across pages (e.g. `folder-plus` for "create workspace").
