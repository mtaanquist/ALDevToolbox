...
- **Generation is synchronous; the build pipeline is the sanctioned async exception.** Workspace/extension generation is read-only against the DB and runs in memory — keep it that way (no queue, no job table). The Artifacts build pipeline is the one place in-process async is blessed: `ProjectBuildImporter.StartBuildAsync` enqueues an in-process `ReleaseImportWorker` job that clones, compiles, and ingests. Reuse *that* worker for new in-process background work; standing up an **external** queue or broker (Redis, a cloud queue) is still a conversation first.
