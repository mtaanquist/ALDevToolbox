...
| `Components/Shared/Archetypes/` | The page frame, one component per handoff structure: `PageHead`, `EmptyState`, `LoadingBlock`, `FilterBar`, the page frames `ListPage`, `DetailPage`, `EditPage` and `LauncherPage`, plus the content archetypes `DocsPage`, `SetupSteps` / `SetupStep` and `ErrorPage`. Pages compose these instead of copying the markup — see "Page archetypes" below. |
