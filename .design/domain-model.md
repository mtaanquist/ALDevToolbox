...
| `workspace_template`| TEXT NOT NULL  | Verbatim content for the `{WorkspaceName}.code-workspace` file. Substitutes `{{paths}}` at generation time with the workspace's folder entries; everything else is editable per template (analyzers, ruleset path, recommended VS Code extensions). Validation: non-empty and must contain `{{paths}}`. |
