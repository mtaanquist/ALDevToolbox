...
* **Not yet landed.** The OIDC handler, sign-in callback, account linking/JIT provisioning, and the `EntraOnly` enforcement — the settings above are inert until those slices merge. The callback path is already fixed as **`/signin-microsoft`** (both admin pages display it as the redirect URI to register in Entra), so the sign-in slice must mount the OIDC handler on exactly that path.
