...
**Multi-tenancy.** Every editable entity carries an `organization_id`. EF Core query filters on `AppDbContext` scope reads to the acting user's organisation; the only paths that bypass them with `IgnoreQueryFilters()` are the ones that *can't* know an organisation yet (login, signup, password reset, bootstrap, and the Microsoft sign-in callback). Cross-org reads from a signed-in session return nothing — verified by `CrossOrgIsolationTests`.
...
* **Client-secret expiry warnings (slice 4).** Entra never tells the app when a client secret lapses — sign-in just starts failing — so `entra_client_secret_expires_at` (a nullable `date`) sits beside the ciphertext on both `system_settings` and `organization_settings`, filled in by whoever pasted the secret. It is advisory and self-reported: null means no warning is possible, and the date is wiped whenever the secret it describes is. `Domain/ValueObjects/EntraSecretExpiry.cs` owns the arithmetic and the two-week window the surfaces share. Both settings pages warn from inside the window and flag a lapsed secret outright; an org whose **own** registration is lapsing also gets a `Needs attention` row on `/admin`, while an org riding on the deployment-wide registration gets a read-only note on its Identity page instead — that secret is a SiteAdmin's to renew, and a row an admin cannot clear does not belong in a column headed "waiting on an admin". Deliberately no scheduler and no email: the warning surfaces where an admin already is.
