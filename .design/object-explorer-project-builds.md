...
- Two recovery actions on the manage page:
  - **Retry the build** — reuse the existing failed-release retry shape (`ReopenForRetryAsync` + re-enqueue), here re-enqueuing a `ProjectBuild` job. Useful when the failure was transient (a flaky clone, a parent artifact that has since been imported).
...
- Worker test — assert a `ProjectBuild` job is durable across a restart (re-enqueued by the reconciler) and that retry re-runs the build in place.
