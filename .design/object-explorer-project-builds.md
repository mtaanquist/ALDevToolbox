...
- **One artifact per build, not per extension.** `ProjectBuildService` resolves a single Microsoft artifact for the highest `application` Major.Minor across the discovered apps, extracts *every* MS `.app` from it (application + platform) into one package-cache dir, and adds the repos' committed `.alpackages/` on top. `alc` resolves each app's deps against that shared cache.
...
- **Manual-symbols recovery shipped — as "persist + rebuild", not "recompile one extension".** The build report's "Supply missing symbols" action (shown for a project release with any `failed` row, partial or fully failed) uploads the missing dependency `.app`(s). They're stored at the *project* level in `oe_project_symbols` (org-scoped, `bytea`) — not transient, not per-extension — so `ProjectBuildService` merges them into the symbol cache (after the committed `.alpackages/`, overwriting a stale copy) on **this** build *and every later one*, including a future auto-build. The action then rebuilds the release in place via the existing `OeProjectBuild` retry path (`ReopenForRebuildAsync` accepts a `ready` partial as well as a `failed` release). This deliberately diverges from the original "re-compile just that one extension" proposal below: compiling one extension needs its sibling deps' symbols anyway (which aren't persisted), so a full rebuild — idempotent, and the same cost as the existing Retry — is both simpler and more useful than an isolated single-app recompile. Retry-build (re-clone HEAD, rebuild in place) shipped earlier.
