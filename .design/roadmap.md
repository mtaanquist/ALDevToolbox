...
- **Import a workspace straight from Azure DevOps / GitHub (PAT).** ✅ *Shipped* — `object-explorer-project-builds.md` ("As built"): define a Customer, point it at repos, clone HEAD with a per-org PAT, resolve symbols from each `app.json`'s `application` version, compile with the runtime-provisioned BC compiler, and ingest as a `customer`-kind Release. (Still gated on outbound network policy: the host needs `dev.azure.com` / `github.com` reachable.) Manual-symbols recovery shipped — upload the missing dependency `.app`(s) from a build's manage page; they're stored against the customer (`oe_customer_symbols`) and merged into the symbol cache on a rebuild, so the build resolves a dep absent from both the repo's `.alpackages/` and any Microsoft artifact (see `object-explorer-project-builds.md`, "Manual-symbols recovery shipped"). The follow-ups that were parked here have all shipped:
  - **Auto-build scheduler.** ✅ *Shipped* — `CustomerAutoBuildScheduler` (mirrors `ReleaseAutoImportScheduler`): per-customer opt-in (`Customer.AutoBuildEnabled`), nightly probes each repo's remote HEAD via `git ls-remote` and queues a new build only when a HEAD moved since the last build's recorded `commit_sha`. Opt out with `DISABLE_CUSTOMER_AUTO_BUILD_SCHEDULER=1`, hour via `CUSTOMER_AUTO_BUILD_HOUR_UTC`. See `object-explorer-project-builds.md` ("Auto-build scheduler").
