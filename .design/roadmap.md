...
- **Import a workspace straight from Azure DevOps / GitHub (PAT).** *Graduated* — designed in `object-explorer-customer-builds.md`: define a Customer, point it at repos, clone HEAD of the default branch with a per-org PAT, resolve symbols from each `app.json`'s `application` version, compile with the BC Development Tools, and ingest the result. Gated on the environment's outbound network policy (the host needs `dev.azure.com` / `github.com` on the allow-list, same as the DVD download-URL feature).
