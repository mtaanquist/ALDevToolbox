...
- **Import a workspace straight from Azure DevOps / GitHub (PAT).** ✅ *Shipped* — `object-explorer-customer-builds.md` ("As built"): define a Customer, point it at repos, clone HEAD with a per-org PAT, resolve symbols from each `app.json`'s `application` version, compile with the runtime-provisioned BC compiler, and ingest as a `customer`-kind Release. (Still gated on outbound network policy: the host needs `dev.azure.com` / `github.com` reachable.) Remaining follow-ups:
  ...
  - **Harden first-party dedup, then free the label globally.** First-party artifact dedup currently keys on the computed label, with the unique label index as a race backstop; only `customer`-kind is exempt from that index today. Storing the artifact version/URL as an explicit dedup key would let us drop label uniqueness for *all* kinds, making the label a pure display string. **The explicit key has to be backfilled onto already-imported releases** — derive it from each existing first-party release's current label using the same deterministic pattern dedup relied on before (`"Business Central {Maj}.{Min} ({CC})"` → version + country), so the new column is populated retroactively in the migration without re-importing anything. Raised during the customer-builds work; parked.
