...
- The org settings page (the same page that hosts the machine-translation key) grows a "Repository access" section: set or clear each PAT, with the view record exposing only `bool HasAzureDevOpsPat` / `bool HasGitHubPat` — never the plaintext.
