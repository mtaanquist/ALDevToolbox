...
- **Manual-symbols recovery shipped — as "persist + rebuild", not "recompile one extension".** The build report's "Supply missing symbols" action (shown for a customer release with any `failed` row, partial or fully failed) uploads the missing dependency `.app`(s). They're stored at the *customer* level in `oe_customer_symbols` (org-scoped, `bytea`) — not transient, not per-extension — so `CustomerBuildService` merges them into the symbol cache (after the committed `.alpackages/`, overwriting a stale copy) on **this** build *and every later one*, including a future auto-build. The action then rebuilds the release in place via the existing `CustomerBuild` retry path (`ReopenForRebuildAsync` accepts a `ready` partial as well as a `failed` release). This deliberately diverges from the original "re-compile just that one extension" proposal below: compiling one extension needs its sibling deps' symbols anyway (which aren't persisted), so a full rebuild — idempotent, and the same cost as the existing Retry — is both simpler and more useful than an isolated single-app recompile. Stored symbols are listed and removable on the customer detail page. Retry-build (re-clone HEAD, rebuild in place) shipped earlier.
...
- **Auto-build scheduler.** ✅ *Shipped* — `CustomerAutoBuildScheduler`, a daily sweep mirroring `ReleaseAutoImportScheduler` (poll-every-minute / run-once-daily, per-org `AmbientOrganizationScope`, opt out with `DISABLE_CUSTOMER_AUTO_BUILD_SCHEDULER=1`, hour via `CUSTOMER_AUTO_BUILD_HOUR_UTC`, default 05:00 UTC). Opt in **per customer** (`Customer.AutoBuildEnabled`, a toggle on the customer detail page). For each enabled customer it probes every repo's remote HEAD with `git ls-remote` (no clone) via `CustomerBuildService.HasRepoChangesSinceLastBuildAsync` and only queues a build (a **new release per change**) when a HEAD differs from the commit recorded in the last build — the `commit_sha` provenance is the dedup key, so the open question below is resolved. A customer with no PAT to probe with never auto-builds. Because the label is now display-only (see the dedup-hardening work in `roadmap.md`), a customer rebuild needs no SHA/date label suffix — the release id is its identity.
